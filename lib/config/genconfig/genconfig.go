...
func (g *GenConfigImp) TopologyFlow() (err error) {
	topologyType, err := g.Prompt.Select("Choose Topology", string(TOPOLOGY_TYPE_HA), string(TOPOLOGY_TYPE_STANDALONE))
	if err != nil {
		return
	}
	switch topologyType {
...
func (g *GenConfigImp) HaFlow() (err error) {
	deployType, err := g.Prompt.Select("Choose type of Deployment Type", string(DEPLOY_TYPE_ON_PREMISE), string(DEPLOY_TYPE_AWS))
	if err != nil {
		return
	}
	switch deployType {
...
func (g *GenConfigImp) AwsFlow() (err error) {
	isPreProvision, err := g.Prompt.Confirm("What is AWS Provision State", "Before Provision", "After Provision")
	if err != nil {
		return
	}
	if isPreProvision {
	...
	return g.AwsHaDeployFlow()
}

func (g *GenConfigImp) AwsHaProvisionFlow() (err error) {
	g.Config = AwsHaProvisionConfigFactory()
	return g.Config.Prompts()
}

func (g *GenConfigImp) AwsHaDeployFlow() (err error) {
	return errors.New("this flow is not applicable for current deployment, use same config used during provision step")
}
