...
func init() {
	...
	policy.MapMethodTo("/chef.automate.api.compliance.reporting.v1.ReportingService/ExportReportManager", "compliance:reporting:reports", "compliance:reports:list", "POST", "/api/v0/compliance/reporting/exportv1", func(unexpandedResource string, input interface{}) string {
		if m, ok := input.(*Query); ok {
			return policy.ExpandParameterizedResource(unexpandedResource, func(want string) string {
				switch want {
				case "id":
