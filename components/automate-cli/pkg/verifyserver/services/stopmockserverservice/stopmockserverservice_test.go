...
func TestNewStopMockServerService(t *testing.T) {
	...
	tests := []struct {
		name string
		args args
		want stopmockserverservice.IStopMockServerService
	}{
		{
			args: args{logger: l},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := stopmockserverservice.NewStopMockServerService(tt.args.logger); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewStopMockServerService() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestStopMockServer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := &stopmockserverservice.StopMockServerService{
				Logger: l,
			}
