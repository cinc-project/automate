...
// createErrorMessage function is converting map into string for better readable format to end user.
...
func (fq *FqdnService) nodeReachable(fqdn, rootCert string, reqNodes []string) (models.Checks, bool) {
	...
	for i := 0; i < 50; i++ {
		...
		// if setNodes become empty, that means we are able to reach all the nodes given in request body.
		if len(setNodes) == 0 {
