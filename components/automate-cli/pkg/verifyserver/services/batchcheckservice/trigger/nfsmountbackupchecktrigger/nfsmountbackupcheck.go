...
func (nbc *NfsBackupConfigCheck) Run(config *models.Config) []models.CheckTriggerResponse {
	...
	if err != nil {
		if nfsMountAPIResponse.Error.Message == "" {
			nfsMountAPIResponse.Error.Message = err.Error()
			nfsMountAPIResponse.Error.Code = 503
		}
		response := constructErrorResult(config, nfsMountAPIResponse.Error.Message, nfsMountAPIResponse.Error.Code)
		return response
