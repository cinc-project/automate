	...
	if err != nil {
		...
		for ip, types := range hostMap {
			for i := 0; i < len(types); i++ {
				finalResult = append(finalResult, checkutils.PrepareTriggerResponse(nil, ip, types[i], resp.Error.Message, constants.HARDWARE_RESOURCE_COUNT, constants.HARDWARE_RESOURCE_COUNT_MSG, true, resp.Error.Code))
			}
		}
		return finalResult
	}
