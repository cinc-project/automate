...
func (t *GenerateReportTask) prepareCustomReportData(ctx context.Context, reqToProcess *report_manager.CustomReportRequest) ([]*reporting.Report, error) {
	...
	for _, report := range reqToProcess.Reports {
		...
		for _, profile := range report.Profiles {
			...
			for _, support := range compProfile.Supports {
				profileData.Supports = append(profileData.Supports, &reporting.Support{
					OsName:        support.OsName,
					OsFamily:      support.OsFamily,
					Release:       support.Release,
					InspecVersion: support.Inspec,
			...
			for _, group := range compProfile.Groups {
				profileData.Groups = append(profileData.Groups, &reporting.Group{
					Id:       group.Id,
					Title:    group.Title,
					Controls: group.Controls,
				})
			}
			for _, attribute := range compProfile.Attributes {
				profileData.Attributes = append(profileData.Attributes, &reporting.Attribute{
					Name: attribute.Name,
					//TODO: add options
				})
			}
			...
			for _, control := range profile.Controls {
				...
				//TODO: Add Control Refs, and Tags
