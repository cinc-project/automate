...
func TestSimpleExport(t *testing.T) {
	...
	assert.NoError(t, err)
	line := []string{"Node1", `2021-11-18 00:00:00 +0000 UTC`, "", "", "test-env", "10.23.149.1", "api.example.com", "ProfileName", `Profile, Title`, "1.2.3", "Profile summary", "ControlID1", "Control Title", "0.12", "false", "passed", "12345.000", "super complex code", "all done", `we don't skip`, "", ""}
	expected := getCSVHeader()
...
func TestExportWaiverData(t *testing.T) {
	...
	line := []string{"Node1", "2021-11-18 00:00:00 +0000 UTC", "", "", "test-env", "10.23.149.1", "api.example.com", "ProfileName", `Profile, Title`, "1.2.3", "Profile summary", "ControlID1", "Control Title", "0.12", "true", "passed", "12345.000", `super complex code`, `all done`, `we don't skip`, "some reason", "some-date"}
	expected := getCSVHeader()
	expected = append(expected, line)
	assert.Equal(t, expected, data)
}
