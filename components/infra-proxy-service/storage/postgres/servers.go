...
func (p *postgres) getServer(ctx context.Context, q querier, id string) (storage.Server, error) {
	...
	err := q.QueryRowContext(ctx,
		`SELECT s.id, s.name, s.fqdn, s.ip_address, s.updated_at, s.created_at,s.credential_id,
		COALESCE((SELECT count(*) FROM orgs o WHERE o.server_id = s.id), 0) AS orgs_count
		FROM servers s
		WHERE s.id = $1`, id).
		Scan(&s.ID, &s.Name, &s.Fqdn, &s.IPAddress, &s.CreatedAt, &s.UpdatedAt, &s.CredentialID, &s.OrgsCount)
	if err != nil {
		return storage.Server{}, p.processError(err)
	}
	return s, nil
