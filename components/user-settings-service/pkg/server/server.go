...
func New(db *postgres.DB, storage storage.Client) *UserSettingsServer {
	userSettingsServer := &UserSettingsServer{}
	err := userSettingsServer.service.SetLogConfig()
	if err != nil {
		userSettingsServer.service.Logger.Error("error setting log config")
	}
...
func (s *UserSettingsServer) GetUserSettings(ctx context.Context,
	...
	s.service.Logger.Debug("GetUserSettings for _default user - response: %v", getDefaultUserSettingsResp)
	if err != nil {
	...
	if containsWhitespace(name) {
		s.service.Logger.Infof("%s contains some whitespace", name)
	...
	s.service.Logger.Debugf("Get settings for connector: %s user: %s", connector, name)
...
func (s *UserSettingsServer) PutUserSettings(ctx context.Context,
	...
	s.service.Logger.Debugf("Put settings for connector: %s user: %s", connector, name)
	...
	for keyFromUser, _ := range userSettings {
		...
		if !found || !allowed(userSettings[keyFromUser].Value, setting.ValidValues) {
			...
			s.service.Logger.Debugf("----->Removing %s from map because it's not a supported setting", keyFromUser)
	...
	s.service.Logger.Debugf("PutUserSettings response: %v", putUserSettingsResp)
...
func (s *UserSettingsServer) DeleteUserSettings(ctx context.Context, req *user_settings.DeleteUserSettingsRequest) (*pb.Empty, error) {
	s.service.Logger.Infof("DeleteUserSettings from server.go")
	name := req.GetUser().GetName()
	if name == "" {
	...
	s.service.Logger.Infof("Put settings for connector: %s user: %s", connector, name)
