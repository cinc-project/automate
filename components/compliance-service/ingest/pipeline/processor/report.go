...
func ComplianceReport(notifierClient notifier.Notifier, automateURL string, isUpdated bool) message.CompliancePipe {
	return func(in <-chan message.Compliance) <-chan message.Compliance {
		return complianceReport(in, notifierClient, automateURL, isUpdated)
	}
}

func complianceReport(in <-chan message.Compliance, notifierClient notifier.Notifier, automateURL string, isUpdated bool) <-chan message.Compliance {
	...
	go func() {
		for msg := range in {
			...
			msg.InspecReport = &relaxting.ESInSpecReport{
				...
				Profiles:         compliance.ReportProfilesFromInSpecProfiles(msg.Report.Profiles, msg.Shared.PerProfileSums, isUpdated),
